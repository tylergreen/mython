...
;; Modified by Tyler Green for Python
;; Created: Nov 2003
;; Modified Dec 2009
;; Keywords: languages
...
;; There is another Python mode, mython-mode.el, used by XEmacs and
...
;; This doesn't implement all the facilities of mython-mode.el, some
...
;; with mython-mode.el in some respects.  For instance, various key
...
;;;###autoload
(add-to-list 'interpreter-mode-alist '("python" . mython-mode))
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.my\\'" . mython-mode))
...
(defvar python-font-lock-keywords
  `(,(rx symbol-start
    ...
    (,(rx symbol-start (group "quote") (1+ space) (group (1+ (or word ?_))))
     (1 font-lock-keyword-face) (2 font-lock-function-name-face))
...
(defvar mython-mode-map
  (let ((map (make-sparse-keymap)))
    ;; Mostly taken from mython-mode.el.
...
(defvar mython-mode-syntax-table
...
(defconst python-space-backslash-table
  (let ((table (copy-syntax-table mython-mode-syntax-table)))
    (modify-syntax-entry ?\\ " " table)
    table)
  "`mython-mode-syntax-table' with backslash given whitespace syntax.")
...
BOS non-nil means point is known to be at beginning of statement."
  (save-excursion
    ...
    (looking-at (rx (and (or "if" "else" "elif" "while" "for" "def"
			     "class" "try" "except" "finally" "with" "quote")
			 symbol-end)))))
	  ...
	  (if open-start
		...
		(current-indentation)
		 ...
		 (if (python-open-block-statement-p t)
		   ...
		   0))))))
       ((bobp) 0)
       ;; Fixme: Like mython-mode.el; not convinced by this.
	     ...
	     (catch 'done
	       (while (cond ((bobp) nil)
			    ...
			    ((/= (current-column) (current-indentation))
			     nil)
			    ;; Indentable comment, like mython-mode.el?
...
(defvar inferior-mython-mode-map
  (let ((map (make-sparse-keymap)))
...
;; Fixme: This should inherit some stuff from `mython-mode', but I'm
;; not sure how much: at least some keybindings, like C-c C-f;
;; syntax?; font-locking, e.g. for triple-quoted strings?
(define-derived-mode inferior-mython-mode comint-mode "Inferior Python"
  "Major mode for interacting with an inferior Python process.
A Python process can be started with \\[run-python].

Hooks `comint-mode-hook' and `inferior-mython-mode-hook' are run in
that order.
...
\\{inferior-mython-mode-map}"
  :group 'python
  (set-syntax-table mython-mode-syntax-table)
...
Runs the hook `inferior-mython-mode-hook' \(after the
...
(defcustom python-source-modes '(mython-mode jython-mode)
...
See variable `python-buffer'.  Starts a new process if necessary."
  ...
  (unless (comint-check-proc python-buffer)
    (run-python nil t))
  (get-buffer-process (or (if (eq major-mode 'inferior-mython-mode)
				  (current-buffer)
				python-buffer))))
...
Otherwise inherits from `mython-mode-syntax-table'.")
...
If these are imported near the beginning of the buffer, `mython-mode'
actually punts to `jython-mode'."
  :type '(repeat string)
  :group 'python)

;; Called from `mython-mode', this causes a recursive call of the
...
`python-jython-packages'."
  ;; The logic is taken from mython-mode.el.
  (save-excursion
    (save-restriction
      (widen)
      (goto-char (point-min))
...
(defun python-current-defun ()
  ...
  (save-excursion
	...
	(when progress
	  (if (looking-at (rx (0+ space) (or "def" "class" "quote") (1+ space)
			      (group (1+ (or word (syntax symbol))))))
...
(eval-after-load "ffap"
  '(push '(mython-mode . python-module-path) ffap-alist))

...
(define-abbrev-table 'mython-mode-abbrev-table ())
...
(defmacro def-python-skeleton (name &rest elements)
  (let* ((name (symbol-name name))
    ...
    `(progn
       ...
       (define-abbrev mython-mode-abbrev-table ,name ""
...
without confirmation."
  ...
  (condition-case data
	...
	(let ((py-mode-map (make-sparse-keymap)) ; it assumes this
	      (features (cons 'mython-mode features))) ; and requires this
	  (brm-init)			; second line of normal recipe
	  (remove-hook 'mython-mode-hook ; undo this from `brm-init'
		       '(lambda () (easy-menu-add brm-menu)))
	  (easy-menu-define
	    python-brm-menu mython-mode-map
	    "Bicycle Repair Man"
	    '("BicycleRepairMan"
...
(defvar python-abbrev-syntax-table
  (copy-syntax-table mython-mode-syntax-table)
...
(defvar python-outline-regexp
  (rx (* space) (or "class" "def" "elif" "else" "except" "finally"
		    "for" "if" "try" "while" "with" "quote")
      symbol-end))

(defcustom python-default-version 2
  "Which version of the language is supported by \\[mython-mode].
...
It stores the value of `python-default-version' when `mython-mode'
was invoked.")

;;;###autoload
(define-derived-mode mython-mode fundamental-mode "Python"
  "Major mode for editing Python files.

The version of the language supported depends on
`python-default-version' (q.v.).  `python-2-mode' and `python-3-mode'
invoke `mython-mode' with the appropriate version set, and are
...
contain Jython code, differs from `mython-mode' only in setting
...
\\{mython-mode-map}"
  ...
  (python-find-imports)
  (unless (boundp 'mython-mode-running)	; kill the recursion from jython-mode
    (let ((mython-mode-running t))
      (python-maybe-jython))))
...
(defcustom mython-mode-hook nil
  "Hook run when entering Python mode."
  :group 'python
  :type 'hook)
(custom-add-option 'mython-mode-hook 'imenu-add-menubar-index)
(custom-add-option 'mython-mode-hook
		   '(lambda ()
		      "Turn off Indent Tabs mode."
		      (setq indent-tabs-mode nil)))
(custom-add-option 'mython-mode-hook 'turn-on-eldoc-mode)
(custom-add-option 'mython-mode-hook 'abbrev-mode)
(custom-add-option 'mython-mode-hook 'python-setup-brm)
(custom-add-option 'mython-mode-hook 'flyspell-prog-mode)
(custom-add-option 'mython-mode-hook 'flymake-mode)
(when (fboundp 'capitalized-words-mode)
  (custom-add-option 'mython-mode-hook 'capitalized-words-mode))

;; This seems to be of limited use since it isn't (can't be)
;; indentation-based.  Also hide-level doesn't seem to work properly.
(dolist (mode '(mython-mode jython-mode))
  (add-to-list 'hs-special-modes-alist
	       (list mode
		     "^\\s-*\\(?:def\\|class\\|if\\|else\\|elif\\|while\\|\
for\\|try\\|except\\|finally\\|with\\|quote\\)\\>"
	       nil "#"
	       (lambda (arg)
		 (python-end-of-block)
		 (skip-chars-backward " \t\n"))
...
;;;###autoload
(define-derived-mode jython-mode mython-mode  "Jython"
  "Major mode for editing Jython files.
Like `mython-mode', but sets up parameters for Jython subprocesses.
Runs `jython-mode-hook' after `mython-mode-hook'."
  :group 'python
