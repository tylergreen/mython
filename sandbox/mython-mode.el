;;; mython-mode.el --- Major mode for editing Mython programs

;; Copyright (C) 1992,1993,1994  Tim Peters

;; Modified for mython by Tyler Green 
;; December 2009

;; Author: 2003-2009 https://launchpad.net/mython-mode
;;         1995-2002 Barry A. Warsaw
;;         1992-1994 Tim Peters
;; Maintainer: tyler.green2@gmail.com
;; Created:    Feb 1992
;; Keywords:   mython languages oop

(defconst py-version "5.1.0"
  "`mython-mode' version number.")

;; This file is part of mython-mode.el.
;;
;; mython-mode.el is free software: you can redistribute it and/or modify it
;; under the terms of the GNU General Public License as published by the Free
;; Software Foundation, either version 3 of the License, or (at your option)
;; any later version.
;;
;; mython-mode.el is distributed in the hope that it will be useful, but
;; WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY
;; or FITNESS FOR A PARTICULAR PURPOSE.  See the GNU General Public License
;; for more details.
;;
;; You should have received a copy of the GNU General Public License along
;; with mython-mode.el.  If not, see <http://www.gnu.org/licenses/>.

;;; Commentary:

;; This is a major mode for editing Mython programs.  It was developed by Tim
...
;; doesn't even use Emacs any more <wink>.  In 2003, mython-mode.el was moved
;; to its own SourceForge project apart from the Mython project, and in 2008
;; it was moved to Launchpad for all project administration.  mython-mode.el
;; is maintained by the volunteers at the mython-mode@mython.org mailing
;; list.

;; mython-mode.el is different than, and pre-dates by many years, the
;; mython.el that comes with FSF Emacs.  We'd like to merge the two modes but
;; have few cycles to do so.  Volunteers are welcome.

;; pdbtrack support contributed by Ken Manheimer, April 2001.  Skip Montanaro
;; has also contributed significantly to mython-mode's development.

;; Please use Launchpad to submit bugs or patches:
;;
;;     https://launchpad.net/mython-mode
...
;; ".py" using mython-mode add the following to your ~/.emacs file (GNU
;; Emacs) or ~/.xemacs/init.el file (XEmacs):
;;    (setq auto-mode-alist (cons '("\\.py$" . mython-mode) auto-mode-alist))
;;    (setq interpreter-mode-alist (cons '("mython" . mython-mode)
;;                                       interpreter-mode-alist))
;;    (autoload 'mython-mode "mython-mode" "Mython editing mode." t)
;;
...
;; As mentioned above, please use the Launchpad mython-mode project for
...
;; mython-mode@mython.org.

;; When in a Mython mode buffer, do a C-h m for more help.  It's doubtful that
...
(defgroup mython nil
  "Support for the Mython programming language, <http://www.mython.org/>"
  :group 'languages
  :prefix "py-")

(defcustom py-tab-always-indent t
  "*Non-nil means TAB in Mython mode should always reindent the current line,
regardless of where in the line point is when the TAB command is used."
  :type 'boolean
  :group 'mython)

(defcustom py-mython-command "mython"
  "*Shell command used to start Mython interpreter."
  :type 'string
  :group 'mython)

(make-obsolete-variable 'py-jmython-command 'py-jython-command)
...
(defcustom py-default-interpreter 'cmython
  "*Which Mython interpreter is used by default.
The value for this variable can be either `cmython' or `jython'.

When the value is `cmython', the variables `py-mython-command' and
`py-mython-command-args' are consulted to determine the interpreter
...
Note that this variable is consulted only the first time that a Mython
mode buffer is visited during an Emacs session.  After that, use
\\[py-toggle-shells] to change the interpreter shell."
  :type '(choice (const :tag "Mython (a.k.a. CMython)" cmython)
                 (const :tag "Jython" jython))
  :group 'mython)

(defcustom py-mython-command-args '("-i")
  "*List of string arguments to be used when starting a Mython shell."
  :type '(repeat string)
  :group 'mython)

(make-obsolete-variable 'py-jmython-command-args 'py-jython-command-args)
...
you're editing someone else's Mython code."
  :type 'integer
  :group 'mython)
...
(defcustom py-smart-indentation t
  "*Should `mython-mode' try to automagically set some indentation variables?
...
to `mython-mode':
    ...
    2. `indent-tabs-mode' is turned off if `py-indent-offset' does not
       ...
       Mython mode).  This means that for newly written code, tabs are
       only inserted in indentation if one tab is one indentation
       level, otherwise only spaces are used.

Note that both these settings occur *after* `mython-mode-hook' is run,
so if you want to defeat the automagic configuration, you must also
set `py-smart-indentation' to nil in your `mython-mode-hook'."
  :type 'boolean
  :group 'mython)
...
(defcustom py-temp-directory
  ...
  "*Directory used for temporary files created by a *Mython* process.
...
(defcustom py-jump-on-exception t
  "*Jump to innermost exception frame in *Mython Output* buffer.
When this variable is non-nil and an exception occurs when running
Mython code synchronously in a subprocess, jump immediately to the
...
e.g. shell buffers and the *Mython* buffer.  When using pdb to debug a
Mython program, pdbtrack notices the pdb prompt and displays the
...
When `mython-mode' tries to calculate the shell to use (either a
CMython or a Jython shell), it looks at the so-called `shebang' line
...
(make-obsolete-variable 'py-jmython-packages 'py-jython-packages)
...
(defvar py-shell-alist
  '(("jython" . 'jython)
    ("mython" . 'cmython))
  "*Alist of interpreters and mython shells. Used by `py-choose-shell'
to select the appropriate mython interpreter mode for a file.")
...
(defcustom py-shell-switch-buffers-on-execute t
  "*Controls switching to the Mython buffer where commands are
  executed.  When non-nil the buffer switches to the Mython buffer, if
...
support for features needed by `mython-mode'.")

;; Face for None, True, False, self, and Ellipsis
(defvar py-pseudo-keyword-face 'py-pseudo-keyword-face
  "Face for pseudo keywords in Mython mode, like self, True, False, Ellipsis.")
...
(defvar mython-font-lock-keywords
  ...
  "Additional expressions to highlight in Mython mode.")
(put 'mython-mode 'font-lock-defaults '(mython-font-lock-keywords))

;; have to bind py-file-queue before installing the kill-emacs-hook
(defvar py-file-queue nil
  "Queue of Mython temp files awaiting execution.
...
(defconst py-stringlit-re
  ...
  "Regular expression matching a Mython string literal.")
...
(defconst py-continued-re
  ...
  (concat
   "\\(" "[^#'\"\n\\]" "\\|" py-stringlit-re "\\)*"
   "\\\\$")
  "Regular expression matching Mython backslash continuation lines.")
...
(defvar mython-mode-abbrev-table nil
  "Abbrev table in use in `mython-mode' buffers.")
(define-abbrev-table 'mython-mode-abbrev-table nil)

(defvar mython-mode-hook nil
  "*Hook called by `mython-mode'.")

(make-obsolete-variable 'jmython-mode-hook 'jython-mode-hook)
...
;; In previous version of mython-mode.el, the hook was incorrectly
;; called py-mode-hook, and was not defvar'd.  Deprecate its use.
(and (fboundp 'make-obsolete-variable)
     (make-obsolete-variable 'py-mode-hook 'mython-mode-hook))

(defvar py-mode-map ()
  "Keymap used in `mython-mode' buffers.")
(if py-mode-map
    nil
  ...
  ;; mython-mode, which forces us to rebind to special commands
  ...
  ;; RET is `newline' and C-j is `newline-and-indent'.  Most Mythoneers
...
(defvar py-mode-output-map nil
  "Keymap used in *Mython Output* buffers.")
(if py-mode-output-map
  ...
  ;; really implement this as *Mython Output* buffer being read-only
  (mapc #' (lambda (key)
...
(defvar py-shell-map nil
  "Keymap used in *Mython* shell buffers.")
(if py-shell-map
    nil
...
(defvar py-mode-syntax-table nil
  "Syntax table used in `mython-mode' buffers.")
...
(defvar py-dotted-expression-syntax-table nil
  "Syntax table used to identify Mython dotted expressions.")
...
(defun py-in-literal (&optional lim)
  "Return non-nil if point is in a Mython literal (a comment or string).
...
(defvar py-menu nil
  "Menu for Mython Mode.
This menu will get created automatically if you have the `easymenu'
package.  Note that the latest X/Emacs releases contain this package.")

(and (py-safe (require 'easymenu) t)
     (easy-menu-define
      py-menu py-mode-map "Mython Mode menu"
      '("Mython"
...
(defvar py-imenu-class-regexp
  ...
  "Regexp for Mython classes for use with the Imenu package."
  )
...
;;   "[ \t]*(\\([-+/a-zA-Z0-9_=,\* \t\n.()\"'#]*\\))"
  ...
  "Regexp for Mython methods/functions for use with the Imenu package."
  )

(defvar py-imenu-method-no-arg-parens '(2 8)
  "Indices into groups of the Mython regexp for use with Imenu.
...
(defvar py-imenu-generic-expression
  ...
  "Generic Mython expression which may be used directly with Imenu.
Used by setting the variable `imenu-generic-expression' to this value.
Also, see the function \\[py-imenu-create-index] for a better
alternative for finding the index.")

;; These next two variables are used when searching for the Mython
;; class/definitions. Just saving some time in accessing the
;; generic-mython-expression, really.
(defvar py-imenu-generic-regexp nil)
(defvar py-imenu-generic-parens nil)


(defun py-imenu-create-index-function ()
  "Mython interface function for the Imenu package.
Finds all Mython classes and functions/methods. Calls function
...
(defun py-imenu-create-index-engine (&optional start-indent)
  "Function for finding Imenu definitions in Mython.

Finds all definitions (classes, methods, or functions) in a Mython
...
at which to continue looking for Mython classes, methods, or
...
(defun py-choose-shell-by-shebang ()
  "Choose CMython or Jython mode by looking at #! on the first line.
...
(defun py-choose-shell-by-import ()
  "Choose CMython or Jython mode based imports.
...
(defun py-choose-shell ()
  "Choose CMython or Jython mode. Returns the appropriate mode function.
...
;      'cmython ;; don't use to py-default-interpreter, because default
;               ;; is only way to choose CMython
      ))


;;;###autoload
(defun mython-mode ()
  "Major mode for editing Mython files.
To submit a problem report, enter `\\[py-submit-bug-report]' from a
`mython-mode' buffer.  Do `\\[py-describe-mode]' for detailed
documentation.  To see what version of `mython-mode' you are running,
enter `\\[py-version]'.

This mode knows about Mython indentation, tokens, comments and
...
py-mython-command\t\tshell command to invoke Mython interpreter
...
py-beep-if-tab-change\t\tring the bell if `tab-width' is changed"
  ...
  (setq major-mode              'mython-mode
        mode-name               "Mython"
        local-abbrev-table      mython-mode-abbrev-table
        font-lock-defaults      '(mython-font-lock-keywords)
        paragraph-separate      "^[ \t]*$"
        paragraph-start         "^[ \t]*$"
...
(make-obsolete 'jmython-mode 'jython-mode)
(defun jython-mode ()
  "Major mode for editing Jython/Jython files.
This is a simple wrapper around `mython-mode'.
It runs `jython-mode-hook' then calls `mython-mode.'
...
;; It's handy to add recognition of Mython files to the
...
;;;###autoload
(when (not (or (rassq 'mython-mode auto-mode-alist)
               (rassq 'jython-mode auto-mode-alist)))
  (push '("\\.py$" . mython-mode) auto-mode-alist))
...

;; Mython subprocess utilities and filters
(defun py-execute-file (proc filename)
  "Send to Mython interpreter process PROC \"execfile('FILENAME')\".
...
;       (comint-scroll-to-bottom-on-output t)
        ...
        (cmd (format "execfile(r'%s') # MYTHON-MODE\n" filename)))
    (unwind-protect
        (save-excursion
          (set-buffer procbuf)
          (goto-char (point-max))
...
(defun py-comint-output-filter-function (string)
  "Watch output for Mython prompt and exec next file waiting in queue.
...
recently visited mython-mode buffer which either has the name of the
current function \(or class) or which defines the function \(or
class).  This is to provide for remote scripts, eg, Zope's 'Script
(Mython)' - put a _copy_ of the script in a buffer named for the
script, and set to mython-mode, and pdbtrack will find it.)"
...
Failing that, we look for the most recently visited mython-mode buffer
    ...
    (let* ((filename (match-string 1 block))
      ...
      (cond ((file-exists-p filename)
            ...
            ((setq funcbuffer (py-pdbtrack-grub-for-buffer funcname lineno))
             (if (string-match "/Script (Mython)$" filename)
...
We walk the buffer-list history for mython-mode buffers that are
...
(defvar py-output-buffer "*Mython Output*")
(make-variable-buffer-local 'py-output-buffer)

;; for toggling between CMython and Jython
(defvar py-which-shell nil)
(defvar py-which-args  py-mython-command-args)
(defvar py-which-bufname "Mython")
(make-variable-buffer-local 'py-which-shell)
(make-variable-buffer-local 'py-which-args)
(make-variable-buffer-local 'py-which-bufname)

(defun py-toggle-shells (arg)
  "Toggles between the CMython and Jython shells.

With positive argument ARG (interactively \\[universal-argument]),
uses the CMython shell, with negative ARG uses the Jython shell, and
with a zero argument, toggles the shell.

Programmatically, ARG can also be one of the symbols `cmython' or
`jython', equivalent to positive arg and negative arg respectively."
  ...
  (cond
   ((equal arg 0)
    ;; toggle
    (if (string-equal py-which-bufname "Mython")
        (setq arg -1)
      (setq arg 1)))
   ((equal arg 'cmython) (setq arg 1))
...
(defun py-shell (&optional argprompt)
  "Start an interactive Mython interpreter in another window.
This is like Shell mode, except that Mython is running in the window
instead of a shell.  See the `Interactive Shell' and `Shell Mode'
sections of the Emacs manual for details, especially for the key
bindings active in the `*Mython*' buffer.

With optional \\[universal-argument], the user is prompted for the
flags to pass to the Mython interpreter.  This has no effect when this
...
Note: You can toggle between using the CMython interpreter and the
Jython interpreter by hitting \\[py-toggle-shells].  This toggles
buffer local variables which control whether all your subshell
interactions happen to the `*Jython*' or `*Mython*' buffers (the
latter is the name used for the CMython buffer).

Warning: Don't use an interactive Mython if you change sys.ps1 or
sys.ps2 from their default values, or if you're running code that
prints `>>> ' or `... ' at the start of a line.  `mython-mode' can't
distinguish your output from Mython's output, and assumes that `>>> '
at the start of a line is a prompt from Mython.  Similarly, the Emacs
Shell mode code assumes that both `>>> ' and `... ' at the start of a
line are Mython prompts.  Bad things can happen if you fool either
mode.

Warning:  If you do any editing *in* the process buffer *while* the
buffer is accepting output from Mython, do NOT attempt to `undo' the
changes.  Some of the output (nowhere near the parts you changed!) may
be lost if you do.  This appears to be an Emacs bug, an unfortunate
interaction between undo and process filters; the same problem exists in
non-Mython process buffers using the default (Emacs-supplied) process
filter."
  (interactive "P")
  ;; Set the default shell if not already set
  (when (null py-which-shell)
  ...
  (let ((args py-which-args))
    ...
    (if (not (equal (buffer-name) "*Mython*"))
...
(defun py-execute-region (start end &optional async)
  "Execute the region in a Mython interpreter.

The region is first copied into a temporary file (in the directory
`py-temp-directory').  If there is no Mython interpreter shell
...
If the Mython interpreter shell is running, the region is execfile()'d
in that shell.  If you try to execute regions too quickly,
`mython-mode' will queue them up and execute them one at a time when
it sees a `>>> ' prompt from Mython.  Each time this happens, the
  ...
  (let* ((proc (get-process py-which-bufname))
         (temp (if (memq 'broken-temp-names py-emacs-features)
                   (let
                     ...
                     (if pid
                         (format "mython-%d-%d" sn pid)
                       (format "mython-%d" sn)))
                 (make-temp-name "mython-")))
    ...
    (cond
     ...
     (async
      ...
      (let* ((buf (generate-new-buffer-name py-output-buffer))
             ...
             (arg (if (string-equal py-which-bufname "Mython")
                      "-u" "")))
     ...
     ;; if the Mython interpreter shell is running, queue it up for
     ;; execution there.
     (proc
      ;; use the existing mython shell
      (save-excursion
        (set-buffer buf)
        (write-region (point-min) (point-max) file nil 'nomsg))
...
(defun py-execute-buffer (&optional async)
  "Send the contents of the buffer to a Mython interpreter.
If the file local variable `py-master-file' is non-nil, execute the
named file instead of the buffer's file.

If there is a *Mython* process buffer it is used.  If a clipping
...
(defun py-execute-import-or-reload (&optional async)
  "Import the current buffer's file in a Mython interpreter.
...
This may be preferable to `\\[py-execute-buffer]' because:
 ...
 - The Mython debugger gets line number information about the functions."
  (interactive "P")
...
(defun py-execute-def-or-class (&optional async)
  "Send the current function or class definition to a Mython interpreter.

If there is a *Mython* process buffer it is used.
...
(defun py-execute-string (string &optional async)
  "Send the argument STRING to a Mython interpreter.

If there is a *Mython* process buffer it is used.

See the `\\[py-execute-region]' docs for an account of some
subtleties, including the use of the optional ASYNC argument."
  (interactive "sExecute Mython command: ")
  (save-excursion
    (set-buffer (get-buffer-create
                 (generate-new-buffer-name " *Mython Command*")))
    (insert string)
    (py-execute-region (point-min) (point-max) async)))
...
(defun py-jump-to-exception (file line)
  "Jump to the Mython code in FILE at LINE."
  (let ((buffer (cond ((string-equal file "<stdin>")
    ...
    ;; Force Mython mode
    (if (not (eq major-mode 'mython-mode))
        (mython-mode))
    (goto-line line)
    (message "Jumping to exception in file %s on line %d" file line)))

(defun py-mouseto-exception (event)
  "Jump to the code which caused the Mython exception at EVENT.
...
(defun py-find-next-exception (start buffer searchdir errwhere)
  "Find the next Mython exception and jump to the code that caused it.
...
stack."
  (interactive "P")
  (let* ((proc (get-process "Mython"))
         (buffer (if proc "*Mython*" py-output-buffer)))
    (if bottom
...
(defun py-indent-line (&optional arg)
  "Fix the indentation of the current line according to Mython rules.
...
from scratch for Mython code.  In general, deletes the whitespace before
  ...
  (save-excursion
    ...
    (let* ((bod (py-point 'bod))
      ...
      (cond
       ...
       ;; specially by the Mython interpreter.
...
new value, so that other Mython buffers are not affected.  With
\\[universal-argument] (programmatically, optional argument GLOBAL),
change the global value of `py-indent-offset'.  This affects all
Mython buffers (that don't have their own buffer-local copy), both
...
set to the difference in indentation between that line and the Mython
...
(defun py-comment-indent-function ()
  "Mython version of `comment-indent-function'."
...
(defun py-shift-region-left (start end &optional count)
  "Shift region of Mython code to the left.
...
(defun py-shift-region-right (start end &optional count)
  "Shift region of Mython code to the right.
The lines from the line containing the start of the current region up
to (but not including) the line containing the end of the region are
...
(defun py-indent-region (start end &optional indent-offset)
  "Reindent a region of Mython code.
...
scratch (that's impossible in Mython), it merely adjusts the existing
...
(defun py-previous-statement (count)
  "Go to the start of the COUNTth preceding Mython statement.
...
(defun py-next-statement (count)
  "Go to the start of next Mython statement.
If the statement at point is the i'th Mython statement, goes to the
...
(defalias 'beginning-of-mython-def-or-class 'py-beginning-of-def-or-class)
...
(defalias 'end-of-mython-def-or-class 'py-end-of-def-or-class)
...
of the region depends on the kind of line at the start:
 ...
 - Else if no prefix argument is given, and the line begins a Mython
...
And in earlier versions of Mython mode, all 3 were tightly connected.
...
forward' string-search commands.  But because Mython `def' and `class'
...
(defun py-pdbtrack-toggle-stack-tracking (arg)
  ...
  (message "%sabled Mython's pdbtrack"
...
(defun py-help-at-point ()
  "Get help from Mython based on the symbol nearest point."
  (interactive)
...
(defun py-describe-mode ()
  "Dump long form of Mython-mode docs."
  (interactive)
  (py-dump-help-string "Major mode for editing Mython files.
Knows about Mython indentation, tokens, comments and continuation lines.
Paragraphs are separated by blank lines only.

Major sections below begin with the string `@'; specific function and
variable docs begin with `->'.

@EXECUTING MYTHON CODE

\\[py-execute-import-or-reload]\timports or reloads the file in the Mython interpreter
\\[py-execute-buffer]\tsends the entire buffer to the Mython interpreter
...
\\[py-shell]\tstarts a Mython interpreter window; this will be used by
\tsubsequent Mython execution commands
...
Although all comment lines are treated alike by Mython, Mython mode
...
character following the initial `#', Mython mode ignores them when
computing the proper indentation for the next line.

Continuation Lines and Statements

The Mython-mode commands generally work on statements instead of on
...
Unlike most programming languages, Mython uses indentation, and only
indentation, to specify block structure.  Hence the indentation supplied
automatically by Mython-mode is just an educated guess:  only you know
...
\t    c = d
\t    _
Mython-mode cannot know whether that's what you intended, or whether
\tif a > 0:
\t    c = d
\t_
was your intent.  In general, Mython-mode either reproduces the
...
suggested indentation, change it to something you do like, and Mython-
...
The remaining `indent' functions apply to a region of Mython code.  They
assume the block structure (equals indentation, in Mython) of the region
...
@LITTLE-KNOWN EMACS COMMANDS PARTICULARLY USEFUL IN MYTHON MODE
...
Entering Mython mode calls with no arguments the value of the variable
`mython-mode-hook', if that value exists and is not nil; for backward
compatibility it also tries `py-mode-hook'; see the `Hooks' section of
the Elisp manual for details.

Obscure:  When mython-mode is first loaded, it looks for all bindings
...
(when (fboundp 'info-lookup-maybe-add-help)
  (info-lookup-maybe-add-help
   :mode 'mython-mode
   :regexp "[a-zA-Z0-9_]+"
   :doc-spec '(("(mython-lib)Module Index")
               ("(mython-lib)Class-Exception-Object Index")
               ("(mython-lib)Function-Method-Variable Index")
               ("(mython-lib)Miscellaneous Index")))
  )
  ...
  (save-excursion
    (let ((here (point))
      ...
      (while (not done)
        ...
        ;; buffer) that begins with a popular Mython keyword, or a
...
KEY is a regular expression describing a Mython keyword.  Skip blank
...
(defun py-current-defun ()
  "Mython value for `add-log-current-defun-function'.
...

(defconst py-help-address "mython-mode@mython.org"
  "Address accepting submission of bug reports.")

(defun py-version ()
  "Echo the current version of `mython-mode' in the minibuffer."
  (interactive)
  (message "Using `mython-mode' version %s" py-version)
  (py-keep-region-active))
...
(defun py-submit-bug-report (enhancement-p)
  "Submit via mail a bug report on `mython-mode'.
...
non-nil) just submit an enhancement request."
  ...
  (let ((reporter-prompt-for-summary-p (if enhancement-p
    ...
    (reporter-submit-bug-report
     ...
     (concat "mython-mode " py-version) ;pkgname
     ;; varlist
...
These are Mython temporary files awaiting execution."
...
;;; see http://mail.mython.org/pipermail/mython-list/2002-May/103189.html
      ...
      (if (looking-at "\\('''\\|\"\"\"\\|'\\|\"\\)\\\\?\n?")
          (setq string-start (match-end 0)
                ...
                delim (buffer-substring-no-properties (match-beginning 1)
                                                      (match-end 1)))
        (error "The parameter start is not the beginning of a mython string"))
...
(defun py-fill-paragraph (&optional justify)
  "Like \\[fill-paragraph], but handle Mython comments and strings.
